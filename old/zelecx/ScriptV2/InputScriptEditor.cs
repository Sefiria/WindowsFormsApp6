...
namespace Script
{
    public partial class InputScriptEditor : Form
    {
        ...
        public InputScriptEditor(InputScript Script, Type EntityType, Action Save)
        {
            ...
            SetProperties(EntityType.GetProperties().Select(x => x.Name).ToList());
        ...
        private void HighlightErrors()
        {
            ...
            for (int y = 0; y < lines.Length; y++)
            {
                ...
                {
                    case KeywordType.Function: Arguments = GetVerbFromSyntax(Functions, word).Arguments; break;
                    case KeywordType.Condition: Arguments = GetVerbFromSyntax(Conditions, word).Arguments; break;
            ...
            for (int i = 0; i < words.Length; i++)
            {
                ...
                {
                    if (!IsPropertyStructure(word) && !IsNonErrorWord(word) && !Variables.Keys.Contains(word))
                    {
