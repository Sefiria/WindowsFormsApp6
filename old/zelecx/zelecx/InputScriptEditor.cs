...
namespace Script
{
    public partial class InputScriptEditor : Form
    {
        ...
        public InputScriptEditor(Cell cell)
        {
            InitializeComponent();

            this.cell = cell;
            Script = new InputScript((this.cell.Container as IExternalInputInfos).Script);
        ...
        public InputScriptEditor(InputScript _behavior, Action<InputScript> _Callback_UpdateScript)
        {
            InitializeComponent();
        ...
        private void btSave_Click(object sender, EventArgs e)
        {
            if (Test(false))
            {
                ...
                if (cell != null) { (cell.Container as IExternalInputInfos).Script = Script; }
                else { Callback_UpdateScript(Script); }
                            ...
                            if (GetTypeFromRaw(words[firstWordNotModifier + 1]) == KeywordType.Normal)
                                if (IsAssignator(words[firstWordNotModifier + 2]))
                                     ...
                                     || IsProperty(words[firstWordNotModifier + 3]))
                                    {
                                        if (GetTypeFromRaw(words[firstWordNotModifier + 3]) == KeywordType.Value)
                                        {
                                            switch (words[firstWordNotModifier + 3])
                                            {
                                                case "=":
