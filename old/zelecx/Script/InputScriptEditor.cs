...
namespace Script
{
    public partial class InputScriptEditor : Form
    {
                    ...
                    else
                    {
                        ...
                        switch (word)
                        {
                            case "if":
                                ...
                                if (!TestCondition.TestPass)
                                    TestCondition = TestStructure(words, "if", "$Normal", "$Comparator", "@VAL@", "then", "$Normal", "$Assignator", "@VAL@");
                                handled = true;
                                break;
                            case "var":
                                ...
                                break;
                            /*default:
                                switch(GetTypeFromRaw(words[firstWordNotModifier], Variables))
                                {
                                    case KeywordType.
                                }
                        ...
                        if(!handled)
                        {
                            if(word.Contains('.'))
                            {
                                ...
                                if (GetTypeFromRaw(val, Variables) == KeywordType.Value && (IsVariable(variable) || IsPublicVariable(variable) || IsSubject(variable)) && IsAssignator(assign) && Properties.Contains(property))
                                {
                                    TestCondition = new TestResult(true, 0, "");
                                }
                            }
                        }
                    }
                            ...
                            if (GetTypeFromRaw(words[firstWordNotModifier + 1]) == KeywordType.Normal)
                                if (IsAssignator(words[firstWordNotModifier + 2]))
                                    if (GetTypeFromRaw(words[firstWordNotModifier + 3]) == KeywordType.Value
                                     ...
                                     || IsProperty(words[firstWordNotModifier + 3]))
                                    {
                                        if (GetTypeFromRaw(words[firstWordNotModifier + 3]) == KeywordType.Value)
                                        {
        ...
        static public bool IsProperty(string word)
        {
            if (word.Contains('.'))
            {
                ...
                if (split.Length == 2 && (IsPublicVariable(split[0]) || IsSubject(split[0])))
                    return InputScript.IsProperty(split[1]);
            }
            return false;
        }
