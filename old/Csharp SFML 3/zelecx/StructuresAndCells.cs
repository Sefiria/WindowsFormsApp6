    ...
    public class StructuresAndCells
    {
        ...
        public class Cell
        {
            ...
            public void DiffuseTension()
            {
                    ...
                    if (Global.MapBounds.Contains(X + RelativeTile.X, Y + RelativeTile.Y))
                    {
                        ...
                        if (cell.CanReceiveTension)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    if (cell.Container.Tension < tAv)
                                        cell.Container.Tension = tAv;
                                }
                                else
                                {
                                    if (cell.Tension < tAv)
                ...
                S = new Point(0, 1);
                D = new Point(1, 0);
                int IdZ = Global.GetMapIdUnderPosition(X + Z.X, Y + Z.Y);
                int IdQ = Global.GetMapIdUnderPosition(X + Q.X, Y + Q.Y);
                int IdS = Global.GetMapIdUnderPosition(X + S.X, Y + S.Y);
                int IdD = Global.GetMapIdUnderPosition(X + D.X, Y + D.Y);
                if (IdZ != 0 && IdZ != 2) NumberOfDivisions++;
                if (IdQ != 0 && IdQ != 2) NumberOfDivisions++;
                if (IdS != 0 && IdS != 2) NumberOfDivisions++;
                if (IdD != 0 && IdD != 2) NumberOfDivisions++;
                DiffuseTo(Z);
                DiffuseTo(Q);
                DiffuseTo(S);
                DiffuseTo(D);
        ...
        public class StructureMergeTool : Structure
        {
            ...
            public override void DeployTensionToCells()
            {
                ...
                if(Global.map[Bounds.X][Bounds.Y - 1].Id > 0)
                    cellsAround.Add(Global.map[Bounds.X][Bounds.Y - 1]);
                if (Global.map[Bounds.X - 1][Bounds.Y].Id > 0)
                ...
                cellsAround.ForEach(cell => { if (cell.Id == 1) WiresAround.Add(cell); });
        ...
        {
            public static int Width = 3, Height = 2;
            public new static int Id = (int)_Main.Tools.AndGate;
            ...
            {
                if (IsOpen)
                    g.FillRectangle(new SolidBrush(Global.ColorOn), Bounds.X * _Main.TileSize + 8, (Bounds.Y - Global.CameraPosition.Y) * _Main.TileSize + 4, _Main.TileSize, _Main.TileSize);
            }
            public override void DeployTensionToCells()
            {
                if (IsOpen)
        ...
        {
            static public int Width = 3, Height = 2;
            public new static int Id = (int)_Main.Tools.XorGate;
            public Cell CellInA, CellInB, CellOut;
            public override bool IsOpen => CellInA.Tension >= MinimumTensionDetected ^ CellInB.Tension >= MinimumTensionDetected;
        ...
        public class StructureGenerator : StructureUserControl
        {
            ...
            {
                if (!Enabled)
                    g.DrawImage(ImagesManager.GetResizedImageIfNeeded(Properties.Resources.GeneratorDisabled), (Bounds.X - Global.CameraPosition.X) * _Main.TileSize, (Bounds.Y - Global.CameraPosition.Y) * _Main.TileSize);
            }
        }
        [Serializable]
        public class StructureSwitch : StructureUserControl
        {
            ...
            static public int Id = (int)_Main.Tools.Switch;
            ...
            public override void DeployTensionToCells()
            {
                if (IsOpen)
                {
                    ...
                    cellsAround.ForEach(cell => { if (Tension < cell.Tension && cell.Id > 0) Tension = cell.Tension; });
            ...
            {
                if (IsOpen)
                    g.FillRectangle(new SolidBrush(Global.ColorOn), (Bounds.X - Global.CameraPosition.X) * _Main.TileSize, (Bounds.Y - Global.CameraPosition.Y) * _Main.TileSize, _Main.TileSize, _Main.TileSize);

            }
            public override void AdditionnalDraw_ApDraw(Graphics g)
            {
                if (!IsOpen)
                    g.DrawImage(Properties.Resources.SwitchClosed, (Bounds.X - Global.CameraPosition.X) * _Main.TileSize, (Bounds.Y - Global.CameraPosition.Y) * _Main.TileSize);
            }
            public override void UpdateOnClick(MouseEventArgs e)
            {
                if (e.Button == MouseButtons.Middle)
        ...
        {
            static public int Width = 3, Height = 2;
            static public int Id = (int)_Main.Tools.Potentiometer;
            ...
            {
                if (Percent > 0 && CellIn.Tension > 0F)
                    g.FillRectangle(new SolidBrush(Global.ColorOn), Bounds.X * _Main.TileSize + 1, (Bounds.Y - Global.CameraPosition.Y) * _Main.TileSize + 1, Percent / 100F * Width * _Main.TileSize - 2, Height * _Main.TileSize - 2);
            }
            public override void UpdateOnClick(MouseEventArgs e)
            {
                if (e.Button == MouseButtons.Middle)
        ...
        public class StructureExternalOutput_1x1 : Structure, IExternalOutputInfos
        {
            ...
            public override void AdditionnalDraw_ApDraw(Graphics g)
            {
                if(Tension >= TensionMinimumToActivate)
                    g.FillRectangle(new SolidBrush(Global.ColorOn), Bounds.X * _Main.TileSize + 2, (Bounds.Y - Global.CameraPosition.Y) * _Main.TileSize + 2, Width * _Main.TileSize - 4, Height * _Main.TileSize - 4);
            }
            public override void DeployTensionToCells()
            {
                // Loose Tension
        ...
        public class StructureExternalOutput_3x3 : Structure, IExternalOutputInfos
        {
            ...
            public override void AdditionnalDraw_ApDraw(Graphics g)
            {
                if (Tension >= TensionMinimumToActivate)
                {
                    Color color = Global.ColorOn;// Step for avoir runtime error
                    color = Color.FromArgb(color.R - 50, color.G - 50, color.B);
                    g.FillRectangle(new SolidBrush(color), Bounds.X * _Main.TileSize + 2, (Bounds.Y - Global.CameraPosition.Y) * _Main.TileSize + 2, Width * _Main.TileSize - 4, Height * _Main.TileSize - 4);
                }
            }
            public override void DeployTensionToCells()
            {
        ...
        public class StructureExternalInput : Structure, IExternalInputInfos
        {
            ...
            public override void DeployTensionToCells()
            {
                GeneratedCells.ForEach(cell => { if(Global.map[cell.x][cell.y].Id > 0 && Tension > 0F) Global.map[cell.x][cell.y].Tension = Tension; });
            }
        }
    }
}
