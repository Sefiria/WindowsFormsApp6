        ...
        private void CreateGraphics()
        {
            ...
            while (count > 0)
            {
                ...
                _px.ForEach(s =>
                {
                    ...
                    {
                        if (!Core.isout(s.x - 1, s.y) && !px.Any(px => px.x == s.x - 1 && px.y == s.y) && Core.RND.Next(4) == 0) px.Add(new vec(s.x - 1, s.y));
                        if (!Core.isout(s.x + 1, s.y) && !px.Any(px => px.x == s.x + 1 && px.y == s.y) && Core.RND.Next(4) == 0) px.Add(new vec(s.x + 1, s.y));
                        if (!Core.isout(s.x, s.y - 1) && !px.Any(px => px.x == s.x && px.y == s.y - 1) && Core.RND.Next(4) == 0) px.Add(new vec(s.x, s.y - 1));
                        if (!Core.isout(s.x, s.y + 1) && !px.Any(px => px.x == s.x && px.y == s.y + 1) && Core.RND.Next(4) == 0) px.Add(new vec(s.x, s.y + 1));
                    }
                });
                count--;
