...
namespace console_v2
{
    public class World
    {
        ...
        internal bool IsntBlocking(vec d, vec c, vec t)
        {
            if (!Dimensions.ContainsKey(d)) return false;
            if (!Dimensions[d].Chunks.ContainsKey(c)) return false;
            return ((int)Dimensions[d].Chunks[c].Tiles[t].Sol).IsntBlockingType();
        }
