...
namespace Voxels {
    ...
    public abstract class VoxelData : IEnumerable<XYZ> {
        ...
        public bool BoundsContains(XYZ xyz) => xyz.X >= 0 && xyz.Y >= 0 && xyz.Z >= 0 && xyz.X < Size.X && xyz.Y < Size.Y && xyz.Z < Size.Z;
        public bool BoundsContains(int x, int y, int z) => BoundsContains(new XYZ(x, y, z));
