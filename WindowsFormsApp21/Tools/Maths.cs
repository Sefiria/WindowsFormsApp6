...
using WindowsFormsApp21.Entities;
using Vector = System.Windows.Vector;

namespace WindowsFormsApp21.Utilities
{
    public static class Maths
    {
        ...
        public static bool CollisionPointBox(float curseur_x, float curseur_y, CollisionSquare box)
        {
            if (curseur_x >= box.X
        ...
        public static bool CollisionBoxBox(CollisionSquare box1, CollisionCircle box2)
        {
            if ((box2.X - box2.R >= box1.X + box1.W) // trop à droite
                || (box2.X + box2.R <= box1.X) // trop à gauche
                || (box2.Y - box2.R >= box1.Y + box1.H)  // trop en bas
                || (box2.Y + box2.R <= box1.Y)) // trop en haut
                return false;
            else
                return true;
        }
        public static bool CollisionBoxBox(CollisionSquare box1, CollisionSquare box2)
        {
        ...
        public static bool CollisionPointCercle(float x, float y, CollisionCircle C)
        {
        ...
        public static bool CollisionDroite(PointF A, PointF B, CollisionCircle C)
        {
            PointF u = PointF.Empty;
            u.X = B.X - A.X;
            u.Y = B.Y - A.Y;
        ...
        public static bool CollisionSegment(float Ax, float Ay, float Bx, float By, CollisionCircle C) => CollisionSegment(new PointF(Ax, Ay), new PointF(Bx, By), C);
        public static bool CollisionSegment(PointF A, PointF B, CollisionCircle C)
        {
        ...
        /// </returns>
        public static int CollisionCercleBox(CollisionCircle C1, CollisionSquare box)
        {
                                            // premier test :
            if (CollisionBoxBox(box, C1) == false)
        ...
        public static bool CollisionCercleCercle(CollisionCircle C1, CollisionCircle C2)
        {
            return Length(C2.Pos.Minus(C1.Pos)) < C1.R + C2.R;
        }
        ...
        public static bool CollisionBoxSegment(CollisionSquare box, PointF a, PointF b)
        {
            float last_t = -1F;
            for (float t = 0F; t <= 1F; t += 0.01F / Math.Min(box.W, box.H))
            {
                if (t == last_t)
                    continue;
                else last_t = t;
                if (box.RectF.Contains(Lerp(a, b, t)))
                    return true;
            }
            return false;
        }
