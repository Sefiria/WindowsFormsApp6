...
namespace WindowsFormsApp25_vx
{
    public class RenderSettings
    {
        ...
        public RenderSettings(int Size, float Yaw, float Pitch)
        {
            this.Size = Size;
            this.Yaw = Yaw;
            this.Pitch = Pitch;
    ...
    {
        protected static void RenderIntoBitmap(VoxelData voxelData, SKBitmap bitmap, RenderSettings renderSettings)
        {
        ...
        protected static SKMatrix44 GetMatrix(VoxelData voxelData, RenderSettings renderSettings)
        {
            var s = renderSettings.Size;
            var r = 1.61803398875f;
            var d = s / (r * voxelData.Size.MaxDimension);
            var tran = SKMatrix44.CreateTranslate(s * 0.5f + Core.Cam.X, s * 0.5f + Core.Cam.Y, Core.Cam.Z);
        ...
        protected static void RenderTriangles(VoxelData voxelData, SKCanvas canvas, MeshSettings settings, RenderSettings renderSettings)
        {
        ...
        protected static void RenderQuads(VoxelData voxelData, int size, SKCanvas canvas, MeshSettings meshSettings, RenderSettings renderSettings)
        {
            var matrix = GetMatrix(voxelData, renderSettings);
        ...
        protected static SKColor ToSKColor(Color c)
        {
